...
std::map<std::string, void (Response::*)(Request &, Request &)>	Response::initMethods()
{
	std::map<std::string, void (Response::*)(Request &, Request &)> map;
...
std::map<std::string, void (Response::*)(Request &, Request &)> Response::_method = Response::initMethods();

// Member functions

void			Response::call(Request & request, Request & requestConf)
{
	// _errorMap = requestConf.getErrorPage();
	// _isAutoIndex = requestConf.getAutoIndex();
	_code = 200;//_code = request.getRet();
	// _hostPort.host = requestConf.getHostPort().host;
	// _hostPort.port = requestConf.getHostPort().port;
	_path = requestConf.getRoot();
...
// Methods
void			Response::getMethod(Request & request, Request & requestConf)
{
	// ResponseHeader	head;
...
void			Response::headMethod(Request & request, Request & requestConf)
{
	ResponseHeader	head;
	(void)request;
...
void			Response::putMethod(Request & request, Request & requestConf)
{
	ResponseHeader	head;
	std::string		content;
...
void			Response::optionsMethod(Request & request, Request & requestConf)
{
	ResponseHeader	head;
	(void)request;
...
void			Response::traceMethod(Request & request, Request & requestConf)
{
	(void)requestConf;
